...
>
> - **`:119` — WRONG.** *Service discovery supplies the SPA's API address.* It never does, in either mode. The browser calls a **relative path**; under `aspire run` the Vite dev server proxies `/api` using `SERVER_HTTP`/`SERVER_HTTPS` injected into the Vite **process**, and at publish the assets are folded into the API container. Service discovery feeds the proxy, not the client. [`SOURCE@microsoft/aspire@b477bdd`, `aspire-ts-cs-starter`; **`EXECUTED 2026-09-24`**, Aspire 13.5.4, Spike B: headless Edge got `200` on a relative `/api` in both `aspire run` and a deployed compose stack, with no API address in `import.meta.env` — `spikes/spike-b-api-address/RESULTS.md`]
