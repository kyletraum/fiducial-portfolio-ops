...
> - **`:90` — INCOMPLETE, AND ABOUT THE WRONG FILE.** The generated `.env` is described as regenerated. Two corrections. (a) `aspire publish` writes it **keys-only** — `Add(key, value: null, …)` then `Save(includeValues: false)` — so a *generated* secret never lands there; what it preserves is any value a **human typed in**, because `SaveKeysOnly` keeps non-empty values it loaded. (b) **The file that carries resolved secrets is `.env.<environment>`**, written by the *deploy* step with `Create` + `includeValues: true` and **rebuilt from scratch every run**, so a rotated credential does propagate. An earlier version of this banner said a rotation would go stale; that was wrong about which file holds the secret. Git-ignore the publish output **directory**, never commit either `.env` variant, and keep the committed key list at `deploy/.env.example`. [`SOURCE@microsoft/aspire@b477bdd EnvFile.cs:143-152`, `DockerComposeEnvironmentResource.cs:546-600`]
> - **The published topology is not the one described here.** One flat network; external endpoints written as a bare `host:container` string with **no host IP**, so Docker binds every interface; health-based `depends_on` silently degraded to `service_started`; and an **Aspire dashboard service** emitted with `restart: always` that no table below lists. [**`EXECUTED 2026-09-24`**, Aspire 13.5.4, Spike C: `compose-dashboard` with `ports: - "18888"` resolves to no host IP; `migrator` → `pg` is `service_started`; `pg` has no healthcheck — `spikes/spike-c-migrator/RESULTS.md`] Base images *can* be digest-pinned via `WithImageSHA256`; only project images need a release-pipeline rewrite.
>
